...
public class GetAccesstoken500ResponseFault {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("faultstring", "detail"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GetAccesstoken500ResponseFault.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in GetAccesstoken500ResponseFault is not found in the empty JSON string", GetAccesstoken500ResponseFault.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GetAccesstoken500ResponseFault.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `GetAccesstoken500ResponseFault` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("faultstring") != null && !jsonObj.get("faultstring").isJsonNull()) && !jsonObj.get("faultstring").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `faultstring` to be a primitive type in the JSON string but got `%s`", jsonObj.get("faultstring").toString()));
      }
