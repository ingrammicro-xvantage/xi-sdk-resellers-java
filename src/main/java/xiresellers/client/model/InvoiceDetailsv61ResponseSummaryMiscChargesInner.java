  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InvoiceDetailsv61ResponseSummaryMiscChargesInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in InvoiceDetailsv61ResponseSummaryMiscChargesInner is not found in the empty JSON string", InvoiceDetailsv61ResponseSummaryMiscChargesInner.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InvoiceDetailsv61ResponseSummaryMiscChargesInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `InvoiceDetailsv61ResponseSummaryMiscChargesInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("isNonMisc") != null && !jsonObj.get("isNonMisc").isJsonNull()) && !jsonObj.get("isNonMisc").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `isNonMisc` to be a primitive type in the JSON string but got `%s`", jsonObj.get("isNonMisc").toString()));
      }
  }
