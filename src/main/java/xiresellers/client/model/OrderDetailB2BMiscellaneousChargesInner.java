  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!OrderDetailB2BMiscellaneousChargesInner.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in OrderDetailB2BMiscellaneousChargesInner is not found in the empty JSON string", OrderDetailB2BMiscellaneousChargesInner.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!OrderDetailB2BMiscellaneousChargesInner.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `OrderDetailB2BMiscellaneousChargesInner` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("chargeAmount") != null && !jsonObj.get("chargeAmount").isJsonNull()) && !jsonObj.get("chargeAmount").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `chargeAmount` to be a primitive type in the JSON string but got `%s`", jsonObj.get("chargeAmount").toString()));
      }
  }
